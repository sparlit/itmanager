...
type TicketSearchResult = { id: string; title: string; status: string; priority: string };
type AssetSearchResult = { id: string; name: string; status: string; category: string };
type StaffSearchResult = { id: string; name: string; role: string; department: string };
type InventorySearchResult = { id: string; name: string; category: string; quantity: number };
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const [ticketsRes, assetsRes, staffRes, inventoryRes] = await Promise.all([
          fetch(`/api/tickets?${params.toString()}`).then((r) => (r.ok ? r.json() : { tickets: [] as TicketSearchResult[] })),
          fetch(`/api/assets?${params.toString()}`).then((r) => (r.ok ? r.json() : { assets: [] as AssetSearchResult[] })),
          fetch(`/api/staff?${params.toString()}`).then((r) => (r.ok ? r.json() : { staff: [] as StaffSearchResult[] })),
          fetch(`/api/inventory?${params.toString()}`).then((r) => (r.ok ? r.json() : { items: [] as InventorySearchResult[] })),
        ]);

        setSearchResults({
          tickets: (ticketsRes.tickets || []).slice(0, 3).map((t: TicketSearchResult) => ({ id: t.id, title: t.title, status: t.status, priority: t.priority })),
          assets: (assetsRes.assets || []).slice(0, 3).map((a: AssetSearchResult) => ({ id: a.id, name: a.name, status: a.status, category: a.category })),
          staff: (staffRes.staff || []).slice(0, 3).map((s: StaffSearchResult) => ({ id: s.id, name: s.name, role: s.role, department: s.department })),
          inventory: (inventoryRes.items || []).slice(0, 3).map((i: InventorySearchResult) => ({ id: i.id, name: i.name, category: i.category, quantity: i.quantity })),
        });
      } catch (err) {
        ...
        <div className="flex items-center gap-1.5">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
              ...
              <DropdownMenuSeparator className="bg-slate-100 dark:bg-slate-700" />
              <DropdownMenuItem className="text-[13px] cursor-pointer rounded-lg" onClick={() => setView("profile-settings")}>Profile Settings</DropdownMenuItem>
              <DropdownMenuItem className="text-[13px] cursor-pointer rounded-lg" onClick={() => setView("preferences")}>Preferences</DropdownMenuItem>
              <DropdownMenuItem className="text-[13px] cursor-pointer rounded-lg" onClick={() => setView("help-center")}>Help Center</DropdownMenuItem>
              <DropdownMenuSeparator className="bg-slate-100 dark:bg-slate-700" />
              <DropdownMenuItem className="text-[13px] text-rose-600 cursor-pointer rounded-lg focus:bg-rose-50 focus:text-rose-700 dark:focus:bg-rose-900/20 dark:focus:text-rose-400" onClick={async () => { await fetch("/api/auth/logout", { method: "POST" }); window.location.href = "/"; }}>Log out</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
