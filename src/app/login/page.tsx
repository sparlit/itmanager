  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setUser(userData);
        setPermissions(ROLE_PERMISSIONS[json.user.role] || []);
      ...
      >
        <Card className="border-slate-800 bg-slate-900/80 backdrop-blur-xl shadow-2xl">
          <CardHeader className="text-center pb-2">
            ...
            <div className="mt-2">
              <Link href="/landing" className="text-sm text-emerald-400 hover:text-emerald-300 transition-colors flex items-center gap-1">
                ← Back to Home
              </Link>
            </div>
          </CardHeader>
