...
import {
  Plus, Search, Package, ArrowDownCircle, ArrowUpCircle,
  ...
  ArrowUpDown, ArrowUp, ArrowDown, X, Warehouse, BarChart3,
} from "lucide-react";
...
export function InventoryView() {
  ...
  const openEdit = (item: InventoryItem) => {
    ...
    form.reset({
      ...
      maxStockLevel: item.maxStockLevel ?? undefined, reorderQty: item.reorderQty ?? undefined,
      unitPrice: item.unitPrice || "", unitCost: item.unitCost || "",
      supplier: item.supplier || "", location: item.location || "",
      binLocation: item.binLocation || "", notes: item.notes || "",
    });
    setAddOpen(true);
  };
  ...
  return (
    <motion.div className="space-y-5" variants={containerVariants} initial="hidden" animate="show">
      ...
      {/* Transaction Dialog */}
      <TransactionEntryDialog open={txnOpen} onOpenChange={setTxnOpen} item={txnPreItem} items={items} preselectType={txnPreType} staff={staff} onSuccess={fetchItems} />
    </motion.div>
  );
}
