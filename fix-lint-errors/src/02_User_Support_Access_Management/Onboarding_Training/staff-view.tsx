...
export function StaffView() {
  ...
  const form = useForm<StaffFormValues>({
    ...
    defaultValues: { name: "", email: "", phone: "", department: "IT", role: "Staff", status: "Active" },
  });
  ...
  return (
    <motion.div className="space-y-5" variants={containerVariants} initial="hidden" animate="show">
      {/* ─── Page Header ───────────────────────────────────────── */}
      <motion.div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between" variants={itemVariants} viewport={{ once: true }}>
        <div>
          <h1 className="text-[20px] font-bold tracking-tight text-slate-900">Team Directory</h1>
          <p className="text-[13px] text-slate-500 mt-0.5">Manage your IT department team members</p>
        </div>
